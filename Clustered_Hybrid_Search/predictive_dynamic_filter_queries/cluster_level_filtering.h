...
namespace clustered_hybrid_search
{
    ...
    class ClusterLevelFilteringHNSW : public hnswlib::HierarchicalNSW<dist_t>
    {
        // Comparator
        ...
        {
            unsigned int key;
            unsigned int estimated_count;
            CountMinSketchMinHash *cms; // pointer to original sketch

            bool operator>(const ClusterInfo &other) const
            {
                return estimated_count > other.estimated_count;
            }
        };
        ...
        using ClusterResult =
            std::vector<std::pair<std::vector<size_t>, size_t>>;
    ...
    public:
        ClusterLevelFilteringHNSW(hnswlib::SpaceInterface<dist_t> *space, size_t max_elements, const std::string &location_of_index, std::unordered_map<tableint, std::vector<std::string>> &meta_data_predicates_)
        ...
        ClusterLevelFilteringHNSW(hnswlib::SpaceInterface<dist_t> *space, size_t max_elements, const std::string &location_of_index)
            : hnswlib::HierarchicalNSW<dist_t>(space, max_elements)
        {
        ...
        void clustering_and_maintaining_sketches(tableint size_of_cluster)
        {
            ...
            for (tableint id = 0; id < this->max_elements_; id++)
            {
                ...
                current_cms.cluster_starting_indexes.push_back(id); // Add the starting index of the cluster NEW ADDITION
...

        {
            ...
            int k_cluster = std::min( this->ef_, mapForCMS.size());
            ClusterResult cluster_start_nodes = top_k_cluster_search(query_attribute, k_cluster);

            CandidateQueue top_candidates; // final top-k results
            ...
            for (const auto &cluster : cluster_start_nodes)
            {
                const auto &vec = cluster.first;      // vector of nodes for this cluster
                int estimated_count = cluster.second; // cluster prediction
                int early_exit_threshold = static_cast<int>(estimated_count * 0.5);

                int local_count = 0; // track satisfying nodes in this cluster

                for (size_t start_node : vec)
                {
                    // Check if node itself satisfies predicate
                    if (filter_id_map[filter_offset + start_node])
                    {
                        dist_t d = this->fstdistfunc_(
                            query_data,
                            this->getDataByInternalId(start_node),
                            this->dist_func_param_);
                        top_candidates.emplace(d, start_node);
                        visited_array[start_node] = visited_array_tag;
                        local_count++;
                    }

                    // Process direct neighbors
                    int *data = (int *)this->get_linklist0(start_node);
                    size_t size = this->getListCount((linklistsizeint *)data);
                    tableint *datal = (tableint *)(data + 1);
                    ...
                    for (size_t j = 0; j < size; ++j)
                    {
                        ...
                        if (filter_id_map[filter_offset + candidate_id])
                        {
                        ...
                        for (size_t k = 0; k < twoHopSize; ++k)
                        {
                            ...
                            if (filter_id_map[filter_offset + candidateIdTwoHop])
                            {
                                ...
                                top_candidates.emplace(dist1, candidateIdTwoHop);
            ...
            // std::cout<<"Top Candidate Size"<<top_candidates.size()<<std::endl;
...
//             // Getting the nearest neighbours size
                              ...
                              size_t batch_start)
        {
            ...
            {
                if (!filter_ids_map[filter_offset + i])
                    continue;
            ...
            std::string filename =
                "/scratch/aa5f25/hnsw/paper/Ground_truth/Q" +
        ...
        void pushTopK(std::priority_queue<ClusterInfo, std::vector<ClusterInfo>, std::greater<ClusterInfo>> &heap, const ClusterInfo &info, size_t K)
        {
            if (heap.size() < K)
            {
                heap.push(info);
            }
            else if (info.estimated_count > heap.top().estimated_count)
            {
                heap.pop();
                heap.push(info);
            }
        ...
        ClusterResult top_k_cluster_search(std::vector<std::string> &query_attribute, int top_K)
        {
            int num_threads = omp_get_max_threads();

            std::vector<std::pair<unsigned int, CountMinSketchMinHash *>> cmsEntries;
            cmsEntries.reserve(mapForCMS.size());

            for (auto &kv : mapForCMS)
                cmsEntries.emplace_back(kv.first, &kv.second);

            std::vector<
                std::priority_queue<
                    ClusterInfo,
                    std::vector<ClusterInfo>,
                    std::greater<ClusterInfo>>>
                localHeaps(num_threads);

#pragma omp parallel
            {
                int tid = omp_get_thread_num();
                auto &localHeap = localHeaps[tid];

#pragma omp for
                for (size_t i = 0; i < cmsEntries.size(); ++i)
                {
                    unsigned int key = cmsEntries[i].first;
                    CountMinSketchMinHash *cms = cmsEntries[i].second;

                    auto estimate = cms->estimate(query_attribute[0]);
                    unsigned int estimated_count =
                        cms->C[estimate.first][estimate.second];

                    pushTopK(localHeap,
                             {key, estimated_count, cms},
                             top_K);
                }
            }

            std::priority_queue<ClusterInfo, std::vector<ClusterInfo>, std::greater<ClusterInfo>> topKHeap;

            for (auto &heap : localHeaps)
            {
                while (!heap.empty())
                {
                    pushTopK(topKHeap, heap.top(), top_K);
                    heap.pop();
                }
            }
            ...
            /* ---- FINAL RESULT WITH FULL VECTOR + ESTIMATED COUNT ---- */
            ClusterResult aggregated_results;

            for (const auto &cluster : result)
            {
                const auto &vec = cluster.cms->cluster_starting_indexes; // full vector of node IDs
                size_t estimated_count = cluster.estimated_count;        // cluster score

                aggregated_results.emplace_back(vec, estimated_count);
            }
