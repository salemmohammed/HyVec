...
#include "../../predictive_dynamic_filter_queries/cluster_level_filtering.h"
#include <thread>
#include <algorithm>
#include <sys/stat.h>
#include <sys/types.h>
                           ...
                           size_t num_threads)
{
    ...
    for (int ef : ef_values)
    {
       ...
       // alg_query_aware->popularityThresoldComputation();
        ...
        // ❗ Preserved your original break behavior
    }

    for (int ef : ef_values)
    {
        if (totalTimePerEfs.find(ef) == totalTimePerEfs.end())
            continue;

        double total_queries =
            static_cast<double>(queries_meta_data.size());
