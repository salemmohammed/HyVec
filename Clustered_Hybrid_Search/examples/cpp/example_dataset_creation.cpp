...
#include "../../dataset_creation/datacreation.h"
#include <thread>
#include <algorithm>
#include <sys/stat.h>
#include <sys/types.h>
...
void batch_process_queries(clustered_hybrid_search::DatasetCreation<float> *alg_query_aware, float *query_data, std::vector<std::vector<std::string>> &queries_meta_data, std::unordered_map<unsigned int, std::vector<std::string>> &meta_data, int dim, size_t num_threads);
int main(int argc, char const *argv[])
{

    /* code */
    int dim = 768;
    std::unordered_map<unsigned int, std::vector<std::string>> meta_data = reading_meta_data("/scratch/aa5f25/datasets/TripClick/documents_full.csv");
    int max_elements = meta_data.size();
    hnswlib::L2Space space(dim);

    clustered_hybrid_search::DatasetCreation<float> *alg_query_aware = new clustered_hybrid_search::DatasetCreation<float>(&space, max_elements, "/scratch/aa5f25/datasets/TripClick/index.bin"); // Load existing index
    ...
    pair<vector<vector<float>>, vector<vector<string>>> query_reading_results = reading_queries("/scratch/aa5f25/datasets/TripClick/Range_date_queries.csv", dim);
...
void batch_process_queries(
    clustered_hybrid_search::DatasetCreation<float> *alg_query_aware,
    float *query_data,
        ...
        for (size_t b = 0; b < num_batches; b++)
        {
            ...
            ParallelFor(
                ...
                [&](size_t row, size_t)
                {
                    ...
                    alg_query_aware->DescendingOrderDistance(query_data + row * dim,
                                                             10000,
                                                             row,
                                                             total_elements,
                                                             start);
