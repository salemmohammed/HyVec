...
void batch_process_queries(clustered_hybrid_search::PredictiveConjuctiveHNSW<float> *alg_query_aware, float *query_data, std::vector<std::pair<std::vector<std::string>, std::pair<int, int>>> &queries_meta_data, std::unordered_map<unsigned int, std::pair<std::vector<std::string>, int>> &meta_data, int dim, size_t num_threads, std::unordered_map<std::string, std::string> &constants);
int main(int argc, char const *argv[])
{

    std::string path_constants = "../examples/constants/conjunctive_query.txt";
    ...
    clustered_hybrid_search::PredictiveConjuctiveHNSW<float> *alg_query_aware = new clustered_hybrid_search::PredictiveConjuctiveHNSW<float>(&space, max_elements, constants.at("INDEX_PATH"), meta_data); // Load existing index

    

    alg_query_aware->clustering_for_cms_and_cdf_filtering(std::stoi(constants.at("CLUSTER_SIZE")));
...
void batch_process_queries(clustered_hybrid_search::PredictiveConjuctiveHNSW<float> *alg_query_aware, float *query_data, std::vector<std::pair<std::vector<std::string>, std::pair<int, int>>> &queries_meta_data, std::unordered_map<unsigned int, std::pair<std::vector<std::string>, int>> &meta_data, int dim, size_t num_threads, std::unordered_map<std::string, std::string> &constants)
{
    ...
    for (int ef : ef_values)
    {
        ...
        for (size_t b = 0; b < num_batches; b++)
        {
            ...
            alg_query_aware->set_popularity_threshold(0.2); // Example threshold, can be tuned or computed based on cluster statistics
