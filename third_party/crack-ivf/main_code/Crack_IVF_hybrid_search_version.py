...
class CrackIVFHybridSearch():
    def __init__(self, nlist=1, default_nprobe=20, niter=0, max_pts=256, seed=42, metric='euclidean', dbname=None, nthreads=None, min_pts=2, pts_crack_thr=64, alpha=0.5, cv_max=2, spread_max=10, size_prcnt_low=10, size_prcnt_high = 90):
        ...
        return 
    def populate_attribute_list(self, list_of_attributes):
        self.attribute_list = list_of_attributes
    ...
    def search(self, queries, query_attributes, k=100):
        DEBUG=False
        ...
        # Filter search results based on attribute equality
        filtered_D = D.copy()
        filtered_I = I.copy()

        for qid in range(len(queries)):
            q_attr = query_attributes[qid]

            for j in range(filtered_I.shape[1]):
                idx = filtered_I[qid, j]

                # Skip already invalid entries
                if idx == -1:
                    continue

                # Remove if attribute predicate fails
                if self.attribute_list[idx] != q_attr:
                    filtered_I[qid, j] = -1
                    filtered_D[qid, j] = np.inf   # use -np.inf for inner-product search
